...
const Search = () => {
  ...
  const handleSearch = (event) => {
    ...
    const url = new URL('http://localhost:8000/papers')
    ...
    fetch(url,
      ...
      .then(
        (result) => {
          if (result.papers) setQuery({ status: 'success', data: result.papers });
  ...
  const handleSearchMockup = (event) => {
    ...
    timerRef.current = setTimeout(() => {
      setQuery({ status: 'success', data: data });
    }, 1000);
  };
  ...
  switch (query.status) {
    case 'success':
      content = (
        <Table data={query.data} columns={columns} />
      );
      break;
    case 'progress':
      content = (
